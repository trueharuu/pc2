    ...
    const MinArray<T, PIECE_SHAPES> &weights)
{
    if (field == NUM_FIELDS - 1)
        return weights[hold];
    if (two_line && fields[field].hash == TWO_LINE_HASH)
        return 0;
    if (pieces.empty())
        return 1 - weights.min;
    ...
    const std::map<int, MinArray<T, PIECE_SHAPES>> &weights, std::map<int, T> &options)
{
    ...
    for (int f : fields[field].edges[front])
    {
        if (killed)
            goto end_push;
        if (!score_perfect_multiple(f, hold, pieces, two_line, weights, options))
            goto end_push;
    }
    ...
    if (front != hold)
    {
        for (int f : fields[field].edges[hold])
        {
            if (killed)
                goto end_push;
            if (!score_perfect_multiple(f, front, pieces, two_line, weights, options))
                goto end_push;
        }
    }
...
ret:
    T sum = 0;
    for (const std::pair<int, T> &p : options)
        sum += p.second;
    return sum;
}
    ...
    volatile T *master_neg, std::mutex *mutex, volatile int *job_num)
{
    ...
    while (true)
    {
        ...
        if (is_hold)
            f = fields[field].edges[hold][j - fields[field].edges[front].size()];
        else
            f = fields[field].edges[front][j];
...
int inner_main(int argc, char **argv)
{
    ...
    std::string weights_path = "weights.txt";
    std::string tree_data_path = "tree_data.js";

    if ((p = option_value(argv, argv + argc, "-G")))
    {
        graph_path = p;
    }

    if ((p = option_value(argv, argv + argc, "-W")))
    {
        weights_path = p;
    }

    if ((p = option_value(argv, argv + argc, "-T")))
    {
        tree_data_path = p;
    }
    ...
    std::ifstream weights(weights_path);
    ...
    if (!weighted_mode)
    {
        ...
        {
            if (itr->first == "default")
                itr++;
            else
                itr = weight_map.erase(itr);
        }
    }
    std::cerr << "\n";

    std::cerr << "Loading graph... " << std::flush;
    std::ifstream graph(graph_path, std::ios::binary);
    ...
    if (boolean_mode)
        std::cerr << "Running boolean mode\n";
    if (decision_mode)
        std::cerr << "Running decision mode\n";
    if (out_mode)
        std::cerr << "Running stdout mode\n";
    if (two_line_mode)
        std::cerr << "Running 2L mode\n";
    if (weighted_mode)
        std::cerr << "Running weighted mode\n";
    std::cerr << '\n';
    ...
    for (int count = 0;; count++)
    {
        ...
        if (s == "-T")
        {
            std::cin >> tree_data_path;
            std::cerr << "Changed tree data path to " << tree_data_path << "\n\n";
            count--;
            continue;
        }
        ...
        if (decision_mode)
        {
            ...
            // std::cerr << std::filesystem::path(tree_data_path) << "\n";
            // std::cerr << std::filesystem::path(tree_data_path).parent_path() << "\n";
            std::filesystem::create_directories(std::filesystem::path(tree_data_path).parent_path());
            std::ofstream fout(tree_data_path);
...
int main(int argc, char **argv)
{
    bool weighted_mode = option_exists(argv, argv + argc, "-w");
    if (weighted_mode)
        return inner_main<double>(argc, argv);
    else
        return inner_main<int>(argc, argv);
}
